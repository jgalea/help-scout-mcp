 ...
 * into the MCP tool result. Help Scout's 422 / 4xx responses sometimes
 * echo input verbatim, which would otherwise flow customer PII into the
 * LLM context (and into anywhere that context is logged or shared).
 *
 * Only `code` and a length-capped `message` are surfaced.
 */
