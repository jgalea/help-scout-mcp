...
**Query Parameters**:
- `embed`: Only `threads` is supported (embeds thread data in the response)
...
Uses **JSON Patch** format (`application/json`). Each operation specifies what to change.

**Request Body** (array of patch operations):
```json
[
  { "op": "replace", "path": "/subject", "value": "Updated subject" },
  { "op": "replace", "path": "/status", "value": "closed" },
  { "op": "replace", "path": "/assignTo", "value": 789 },
  { "op": "remove", "path": "/assignTo" }
]
```

**Supported paths**: `/subject`, `/status`, `/assignTo`, `/draft`

**Response**: `204 No Content`

> **Important**: Tags and custom fields are NOT updated via this PATCH endpoint. Use the dedicated endpoints below.

##### Update Conversation Tags
**Endpoint**: `PUT /v2/conversations/{id}/tags`

Replaces all tags on a conversation. Send an empty array to remove all tags.
...
**Endpoint**: `PUT /v2/conversations/{id}/fields`

Replaces custom field values on a conversation.
