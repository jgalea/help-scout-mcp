...
export class ResourceHandler {
    ...
    async getInboxesResource(params) {
        try {
    ...
    async getConversationsResource(params) {
        try {
            const queryParams = {
                page: parseInt(params.page || '1', 10),
                size: parseInt(params.size || '50', 10),
            ...
            if (params.mailbox)
                queryParams.mailbox = params.mailbox;
            if (params.status)
                queryParams.status = params.status;
            if (params.tag)
                queryParams.tag = params.tag;
            if (params.modifiedSince)
                queryParams.modifiedSince = params.modifiedSince;
            const response = await helpScoutClient.get('/conversations', queryParams);
    ...
    async getThreadsResource(params) {
        const conversationId = params.conversationId;
        if (!conversationId) {
            throw new Error('conversationId parameter is required for threads resource');
        }
        try {
            const response = await helpScoutClient.get(`/conversations/${conversationId}/threads`, {
                page: parseInt(params.page || '1', 10),
                size: parseInt(params.size || '50', 10),
            });
