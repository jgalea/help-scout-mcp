...
const REPORT_TOOL_DESCRIPTIONS = {
...
export class ReportsToolHandler extends Injectable {
        ...
        try {
            ...
            if (input.collections && input.collections.length > 0) {
                ...
                for (const collectionId of input.collections) {
                    ...
                    const response = await helpScoutDocsClient.get(`/collections/${collectionId}/articles`, {
                        page: 1,
                        pageSize: 100,
                        sort: 'popularity',
                        order: 'desc',
                ...
                for (const siteId of input.sites) {
                    ...
                    if (collectionsResponse.items) {
                        for (const collection of collectionsResponse.items) {
                            ...
                            const articlesResponse = await helpScoutDocsClient.get(`/collections/${collection.id}/articles`, {
                                page: 1,
                                pageSize: 100,
                                sort: 'popularity',
                                order: 'desc',
            ...
            const sortedArticles = allArticles
                .sort((a, b) => (b.viewCount || b.popularity || 0) - (a.viewCount || a.popularity || 0))
                .slice(0, input.limit);
            ...
            const topArticles = isVerbose(args) ? sortedArticles : sortedArticles.map((article) => ({
                id: article.id,
                title: article.name,
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            topArticles: isVerbose(args) ? topArticles : (input.includeStats ? topArticles : topArticles.map((a) => ({
                                id: a.id,
    ...
    async getReport(args) {
        ...
        try {
            ...
            if (input.type === 'docs') {
                let response = null;
                try {
                    response = await reportsApiClient.getReport(endpoint, params);
                }
                catch (error) {
            ...
            const response = await reportsApiClient.getReport(endpoint, params);
            return {
                content: [
                    {
                        type: 'text',
    ...
    async getHappinessRatings(args) {
        ...
        try {
            ...
            const response = await reportsApiClient.getReport('/reports/happiness/ratings', params);
            return {
                content: [
                    {
                        type: 'text',
