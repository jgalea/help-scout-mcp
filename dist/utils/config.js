...
export const config = {
    helpscout: {
        ...
        replySpacing: (process.env.HELPSCOUT_REPLY_SPACING === 'compact' ? 'compact' : 'relaxed'),
...
export function isVerbose(args) {
    if (args && typeof args === 'object' && 'verbose' in args && typeof args.verbose === 'boolean') {
        return args.verbose;
    }
...
export function validateConfig() {
    ...
    if (process.env.HELPSCOUT_API_KEY?.startsWith('Bearer ')) {
        throw new Error('Personal Access Tokens are no longer supported.\n\n' +
    ...
    if (config.helpscout.baseUrl && !config.helpscout.baseUrl.startsWith('https://')) {
        throw new Error('Security Error: HELPSCOUT_BASE_URL must use HTTPS to protect credentials in transit.\n' +
