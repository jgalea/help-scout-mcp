...
export const StructuredConversationFilterInputSchema = z.object({
    ...
    limit: z.number().min(1).max(200).default(50),
    cursor: z.string().optional(),
}).refine((data) => !!(data.assignedTo !== undefined || data.folderId !== undefined || data.customerIds !== undefined || data.conversationNumber !== undefined || (data.sortBy && ['waitingSince', 'customerName', 'customerEmail'].includes(data.sortBy))), { message: 'Must use at least one unique field: assignedTo, folderId, customerIds, conversationNumber, or unique sorting. For content search, use searchConversations with searchTerms.' });
// Response Types
...
}).refine((data) => !!(data.subject !== undefined || data.status !== undefined || data.assignTo !== undefined || data.tags !== undefined || data.customFields !== undefined), { message: 'At least one field to update must be provided (subject, status, assignTo, tags, or customFields).' });
